...
- **日期**：2026-06-26
- **狀態**：已實作（PR #33）。本文件已同步「實作後調整」（見 §12）。
- **分支**：`feat/autopass-drivingexpense-applications`
- **來源需求**：Notion `[駕駛中心 v8.3] 自動繳收尾盤點` — 營運後台 #6「後台顯示自動繳申請清單」(P2) + #7「後台開放營運設定服務查繳週期」(P2)

## 1. 背景與目的

`autopass-demo` 是純前端、全 mock 的 Autopass 內部後台原型。本次新增一個**營運後台頁面「通行費申請單」**，讓營運人員：

1. 查看所有自動繳申請清單（哪個帳號、申請了哪個服務、用什麼資料、何時申請、目前的查繳週期）。
...
- 頁面外觀沿用既有管理頁（以 `StoreManagement` 為範本）的 `<Paper>` shell + Mantine `<Table>` + 搜尋 + `Pagination` 慣例；列上操作 icon 沿用「業者管理」列表的 `<ActionIcon>` 用法。
...
- 新頁面「通行費申請單」：表格顯示申請清單。
- 每列可透過編輯 icon 開 Modal，修改該申請的查繳週期；變更存於記憶體並即時生效。
- 側欄導覽新增入口、路由接線。
...
export const mockApplications: AutopassApplication[] = [ /* 8 筆 */ ]
```

需求：

- 共 8 筆，**涵蓋全部 5 種 `ServiceType`**（交通罰緩已下架移除，見 §12），個人與法人並存。
- 查繳週期分布要含 `雙週繳`（呼應 v8.3 ETC 改雙週繳）。
- 每筆 `queryData` 的 key 必須與 `SERVICE_QUERY_FIELDS[serviceType]` 完全對齊（例如 `compulsory-insurance-fine` 要有 `plateNumber` + `vehicleType`；`traffic-fine-corporate` 要有 `idNumber` + `plateNumber` + `vehicleType`）。
...
- 在「通行費自動繳」群組、`歷史任務` 之後新增一個 `<NavLink label="通行費申請單">`，icon 用 `IconFileText`（`@tabler/icons-react`），active key `'autopass-applications'`，沿用 `navItemStyles`。
...
- **Header**：`<Group justify="space-between" px="20px" py="24px">`，左 `<Title order={2}>通行費申請單</Title>`，**右側不放任何按鈕**。
...
| 欄 | 內容 | 來源 |
|---|---|---|
| Email | Email 完整值 | `app.userEmail` |
| 申請服務 | 服務名稱 | `SERVICE_META[app.serviceType].label` |
| 查繳週期 | 週期值（Badge） | 合併後的 `billingCycle`（含 override） |
...
| 操作 | 編輯 icon（pencil） | `IconPencil` 包在 `<ActionIcon variant="transparent" size={20}>`（沿用業者管理列表用法），開 `EditCycleModal` |
...
- 觸發：列上編輯 icon（pencil）→ 設定 `editingId`，開 Modal。
- 內容：
...
| `src/components/Navigation.tsx` | `NavigationView`、`getActiveKey`、新 `NavLink`（label「通行費申請單」） |

## 10. 驗收條件

1. 側欄「通行費自動繳」群組出現「通行費申請單」，點擊導到 `/autopass/drivingexpense-applications` 並 highlight。
2. 表格列出 mock 申請，欄位齊全（Email / 申請服務 / 查繳週期 / 申請資料 / 申請時間 / 操作），申請資料依服務動態顯示對的欄位、值完整不遮罩。
3. 搜尋可用 Email / 車牌過濾，服務別 `Select` 可篩選（與搜尋為 AND）；分頁正常。
4. 點某列編輯 icon 開 Modal，改週期後儲存 → 該列查繳週期即時更新、跳成功 toast，重新整理才重置。
...
實作期間依需求做了以下調整，本文件已同步：

1. **頁面 / 導覽命名**：初版「自動繳申請」→ 定稿「**通行費申請單**」（頁標題 + 側欄項目）。
2. **第一欄欄名**：「駕駛中心帳號」→「**Email**」。
3. **欄位順序**：「查繳週期」移到「申請服務」之後（Email / 申請服務 / 查繳週期 / 申請資料 / 申請時間 / 操作）。
4. **操作互動**：「編輯」文字連結 → **pencil ActionIcon**（沿用業者管理列表的 `<ActionIcon variant="transparent" size={20}>` + `IconPencil`）。
5. **移除交通罰緩服務**：`traffic-fine-personal` / `traffic-fine-corporate` 已於 Prod 下架，連同其 mock 申請一併移除 → `ServiceType` 由 7 種降為 **5 種**，本頁 mock 由 10 筆降為 **8 筆**。（此為跨功能變更，同 PR 一併處理；保留違反強制險罰緩、汽燃費逾期罰緩。）
