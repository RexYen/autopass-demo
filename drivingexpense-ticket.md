...
證件欄位 label 個人顯示「證件號碼」、法人顯示「統一編號」（`TicketCard.tsx` 的 `idLabel`）；證件號碼／統編目前明碼顯示，未套用 mask（`utils/mask.ts` 的 `maskId` 現階段未被使用）。出生年月日因目前沒有任何服務的 `SERVICE_QUERY_FIELDS` 含 `birthDate`，卡片／詳情 Hero／歷史搜尋欄皆不會出現（程式碼保留顯示分支備用）。
