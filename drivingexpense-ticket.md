...
**定位**：自動繳「申請」的營運清單——用戶替某服務開通自動繳的申請紀錄，營運可逐筆調整查繳頻率。`billingCycle`（週繳／雙週繳／月繳／年繳，單一來源 `BILLING_CYCLES`）與 `Ticket.cycle`（週期實例字串，如 `2026/W18`）是不同概念，不可混用。原始設計討論見 `docs/superpowers/specs/2026-06-26-autopass-applications-design.md`（歷史文件，現行以本節與程式碼為準）。

- **表格欄位**：Email / 申請服務（`SERVICE_META[serviceType].label`）/ 查繳週期（藍底 Badge）/ 申請資料（依 `SERVICE_QUERY_FIELDS[serviceType]` 動態列出「label：值」）/ 申請時間（`YYYY-MM-DD hh:mm`）/ 操作（pencil `ActionIcon`）。
...
- **編輯查繳週期 Modal**：帳號／服務唯讀＋週期 Select；「儲存」寫入 `cycleOverrides`（in-memory override，同全 app demo 慣例），成功 toast 走 `useNotification().showSuccess`。
- **Mock**：`src/data/autopassApplicationsMock.ts`，8 筆，涵蓋全部 5 種 `ServiceType`。
