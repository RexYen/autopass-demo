...
- **表格欄位**（依序）：Email / 車牌號碼 / 車種 / 證件號碼／統編（表頭取 `QUERY_FIELD_META` label）/ 申請服務（`SERVICE_META[serviceType].label`）/ 查繳週期（藍底 Badge，所有服務皆有值；ETC 列 Badge 旁內嵌編輯鉛筆）/ 申請時間（`YYYY-MM-DD hh:mm`）。無獨立操作欄。
...
- **編輯查繳週期 Modal**：由 ETC 列查繳週期旁的鉛筆開啟；帳號／服務唯讀＋週期 Select（選項＝`BILLING_CYCLES`）；「儲存」寫入 `cycleOverrides`（in-memory override，同全 app demo 慣例），成功 toast 走 `useNotification().showSuccess`。
