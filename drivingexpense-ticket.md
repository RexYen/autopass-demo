...
├── 📝 通行費申請單     /autopass/drivingexpense-applications   ← 自動繳申請清單 + 可編輯查繳週期（`AutopassApplications.tsx` + `autopassApplicationsMock.ts`）
├── 📤 對帳匯出       /autopass/export        ← ⚠️ 規劃中，尚未實作（無頁面、無路由）
└── 🧪 狀態 Preview   /preview                ← 前端參考用，**不掛 nav**，靠 URL 進入
```

路由由 React Router（`BrowserRouter`）管理，網址是真實 path；`App.tsx` 以 `pathToView(pathname)` 從 URL 反推 `currentView`，僅用來標示側欄 active 項目。側欄中「通行費自動繳」為可收合群組，位於群組內頁面時預設展開。任務詳情為 Drawer，無深度連結。
...
[pending-query] ──線下查 → 點「回填查詢結果」開 Modal──┐
   ...
   │
   └─[3] 查到金額（Step 1 填線上金額）──→ 按「送出請款」進 Modal 第二步：模擬請款結果 ──→ 顯示結果
         ├ 成功 → [invoice-success]
...
| 查詢失敗 → 資料錯誤 | `query-failed` | reason `data-error` | 寄信引導用戶更新資料（`query-failed-v1`） | 終結（等下一週期） |
| 查詢失敗 → eTag 已綁定（僅 ETC） | `query-failed` | reason `etag-bound` | 寄信引導用戶更新資料（`query-failed-v1`） | 終結 |
| 查到待繳 → 全額可線上代繳（請款成功） | `invoice-success` | outcome `online-full` | 同步請款成功 | 線下代繳 → 確認已代繳 |
| 查到待繳 → 全額可線上代繳（請款失敗 + 重試）| `invoice-failed` | outcome `online-full` | 同步請款失敗、push 失敗 invoice、寄信引導更新付款方式（`invoice-failed-v1`） | 終結；X 天後排程新 ticket |
| 查到待繳 → 全額可線上代繳（請款失敗 + 不重試）| `invoice-failed` | outcome `online-full` | 同上 | 終結，不再追蹤 |
| 查到待繳 → 部分需臨櫃繳費（同上三組）| `invoice-success` / `invoice-failed` | outcome `online-mixed` | 同步請款（僅線上部分）＋寄臨櫃繳費通知信 | 同上 |

兩種「查到待繳」資料層完全相同（`amount: X`），差別只在 `outcome` 與 toast 文案。

> **Demo 中「寄信」的呈現**：既有 mock 票的 `emailLogs` 帶有上表對應信件（`query-failed-v1` / `invoice-failed-v1`；另有一張 `no-fee` 票帶「本期無待繳項目」`no-fee-v1`，屬 mock 示意、上表未列為系統動作），Modal 文案與 toast 也會提示將寄信；但操作當下真正寫入 `emailOverrides` 的只有「確認已代繳」的 `paid-v1`（見 4.1「Demo 用本地狀態」），其餘路徑不新增 EmailLog。
...
- **服務 Tabs（4 個）**：ETC 通行費 / 汽燃費 / 汽燃費逾期罰鍰 / 違反強制險罰鍰。每個 tab 計數即時顯示。
- **搜尋**：
  - 查繳任務頁（current）：單一搜尋框，命中 Email + 該 tab 對應的查詢欄位（車牌、身分證／統編、出生年月日、車種）；Ticket ID 仍可搜尋但不寫在 placeholder 提示中。與歷史頁一致為**送出制**：按「搜尋」才套用、「重設」清空（狀態篩選亦同）。
  - 歷史任務頁（history）：搜尋欄拆成獨立輸入：**Email**（永遠顯示）+ 依當前 tab 動態出現 **車牌 / 證件號碼（只比對個人票）/ 統一編號（只比對法人票）/ 出生年月日**（TextInput）與 **車種**（Select，含汽車／機車／大型重型機車／拖車），另固定提供 **建立月份起迄**（起始月份～結束月份兩個 Select，選項取自現有資料的月份）。各欄位之間是 AND，按「搜尋」才套用；切 tab 時隱藏欄位的值會被忽略但保留在 state 中。
...
| 上半（flex 主體） | Email、（依 `SERVICE_QUERY_FIELDS` 動態）車牌／身分證／出生／車種、查繳平台（非街口支付時為 `platformUrl` 外連，目前僅 ETC 的遠通電收；街口支付顯示純文字、無連結） |
...
| --- | --- | --- | --- |
| `pending-query` | 回填查詢結果 | 開兩步 Modal（3 選 1：需繳費（預設、列首位）／無需繳費／查詢失敗 → 若需繳費進結果步驟） | 加備註 |
...
- 線下查繳平台（非街口支付時附 `platformUrl` 外連，目前僅 ETC 的遠通電收；街口支付顯示純文字、無連結）
- Radio 三層（臨櫃相關子選項僅 `compulsory-insurance-fine` 顯示，其他服務固定視為無應繳費用／全額線上代繳）：
  - 需繳費（預設選取、列首位）→ 全額可線上代繳 / 部分需臨櫃繳費（NumberInput 填線上金額）（子選項僅強制險出現）
  - 無需繳費 → 無應繳費用 / 需臨櫃繳費（對應 outcome `counter-only`）（子選項僅強制險出現）
...
- 成功面板（綠）：金額摘要 + remit hint → 按「下一步」寫入 `status: invoice-success`，並自動接開「確認已代繳」Modal（`openConfirmPaidAfter`），讓操作員可一路完成代繳確認
- 失敗面板（紅）：顯示「請款失敗」＋金額（面板不顯示失敗原因；demo 以固定字串「信用卡授權失敗（demo 模擬）」寫入 `InvoiceOrder.failReason`，目前 UI 無處渲染該欄位）+ Radio「**幾天後重新查詢**（預設，NumberInput X 天，預設 7）/ **不重新嘗試**」→ 確認結案後 `status: invoice-failed`
  ...
  - 「不重新嘗試」直接結案 toast
- 底部按鈕：右下「關閉」+「下一步」（成功路徑）或「確認結案」（失敗路徑）；已無「返回修改」按鈕，要改 Step 1 選項需關閉 Modal 重開
...
沒有後端，重整即還原。另注意 override **不更新 `updatedAt`**：demo 流轉狀態後，卡片「最後更新」與詳情合成事件的推導時間仍是 mock 原值，可能早於實際操作時間（已知 demo 限制）。
...
> 早期版本的「FlowStepper（查詢→請款→代繳→結案）」、「failReason banner（紅色 Alert）」、「繳費證明 Tab + lightbox」均已移除。金額不在 Hero 顯示，要看金額看 Activity 的 `query-result` 事件副標（「線上金額 NT$ X,XXX」）。

#### Activity 區（ClickUp 風格時間軸）

**版面結構**：

- **Section header**：`<IconActivity>` + 「任務歷程」標題，不附事件計數
...
每則事件的主要視覺為 **執行者（粗體深色；目前為寫死的「系統」「Lance」或備註作者）+ 右側時間**；動作與細節以灰字副標顯示（如「建立工單 · 服務 · cycle」「回填查詢結果 - <選擇項> · 線上金額 NT$ X,XXX」「新增備註：<內容>」）。
...
| `note` | `notes[]` | `新增備註` | 備註內容（純文字副標「新增備註：<內容>」，無灰底 block） |
| `email` | `emailLogs[]` | `寄送通知信` / `通知信寄送失敗` | subject · 模板 · 觸發狀態 |

**`query-result` 是合成事件**：mock 沒記錄回填的時戳，因此推導：取第一筆 invoice 或 email 的時戳，往前推 1 分鐘，讓「回填 → 寄信／請款」的時序合理；票上沒有任何 invoice / email 時 fallback 用 `ticket.updatedAt`。對 `query-failed` 票會額外帶 `queryFailureReason` 進 event，副標顯示具體原因。
...
證件欄位 label 個人顯示「證件號碼」、法人顯示「統一編號」（`TicketCard.tsx` 的 `idLabel`）；證件號碼／統編明碼顯示，demo 不做遮罩（原 `utils/mask.ts` 從未接線，已移除）。出生年月日因目前沒有任何服務的 `SERVICE_QUERY_FIELDS` 含 `birthDate`，卡片／詳情 Hero／歷史搜尋欄皆不會出現（程式碼保留顯示分支備用）。
...
| --- | --- | --- | :---: | :---: |
| `pending-query` | 回填查詢結果 | 開兩步驟 `QueryResultModal`（3 選 1 → 需繳費進結果步驟） | 開 `AutopassTicketDetail` Drawer | 開 `AddNoteModal` |
...
### 4.5 通行費申請單（`AutopassApplications.tsx`）

**定位**：自動繳「申請」的營運清單——用戶替某服務開通自動繳的申請紀錄，營運可逐筆調整查繳頻率。`billingCycle`（每月兩次／每週，單一來源 `BILLING_CYCLES`；所有申請皆有值，僅 ETC 開放編輯、預設每月兩次，對應 PRD v9.1.1 §2.5）與 `Ticket.cycle`（週期實例字串，如 `2026/W18`）是不同概念，不可混用。`AutopassApplication` 的車牌／證件號碼／車種／查繳週期為必有值的固定欄位（不用 `queryData` map），UI 不出現「—」。原始設計討論見 `docs/superpowers/specs/2026-06-26-autopass-applications-design.md`（歷史文件，現行以本節與程式碼為準）。

- **表格欄位**（依序）：Email / 車牌號碼 / 車種 / 證件號碼／統編（表頭取 `QUERY_FIELD_META` label）/ 申請服務（`SERVICE_META[serviceType].label`）/ 查繳週期（藍底 Badge，所有服務皆有值；ETC 列 Badge 旁內嵌編輯 icon＝`IconEdit`）/ 申請時間（`YYYY-MM-DD hh:mm`）。無獨立操作欄。
- **排序**：清單固定依申請時間 近→遠（PRD v9.1.1 §2.1）。
- **搜尋／篩選（即時制，與 4.1 的送出制不同）**：關鍵字比對 Email／車牌號碼／證件號碼（統編）；「請選擇服務類型」Select 以 `SERVICE_META` label 去重（個人／法人汽燃費合併為「汽燃費」一項）、「請選擇查繳週期」Select（`BILLING_CYCLES`），兩者皆可清除；另有「重設」鈕一次清空搜尋與兩個篩選；任何變更即過濾並跳回第 1 頁。
- **分頁**：每頁 10 筆，footer 顯示「顯示 X - Y 筆，共 N 筆」。
- **編輯查繳週期 Modal**：由 ETC 列查繳週期旁的編輯 icon 開啟；標題「編輯查繳週期」下方一行副標顯示「帳號 · 服務」（唯讀灰字），body 僅查繳週期 Select（選項＝`BILLING_CYCLES`）；「儲存」寫入 `cycleOverrides`（in-memory override，同全 app demo 慣例），成功 toast 走 `useNotification().showSuccess`。
- **Mock**：`src/data/autopassApplicationsMock.ts`，8 筆，涵蓋全部 5 種 `ServiceType`，每筆皆有車牌／證件號碼／車種／查繳週期。

## 5. 資料模型（mock）

詳細型別在 `src/types/autopass.ts`，mock 資料在 `src/data/autopassMock.ts`（查繳 ticket）與 `src/data/autopassApplicationsMock.ts`（通行費申請單頁的 `AutopassApplication` 清單，查繳週期為 `BillingCycle`）。核心結構：
...
每個服務 tab 都備有四種狀態示意（待查詢 / 請款成功 / 請款失敗 / 查詢失敗，後兩者為終結態、落在歷史任務）；汽燃費、ETC 兩個 tab 額外有「法人」票各一張（分別是 `fuel-fee-corporate` 與 `etc-toll`）。混合 outcome（`online-mixed`）與 `no-fee`（`counter-only`）目前 mock 均無示意資料。歷史任務涵蓋 `paid` / `no-fee`（`no-fee`）/ `query-failed`（其中一張 ETC 用 `etag-bound`）/ `invoice-failed` 等示意。

### 5.5 新增 `ServiceType` 檢查清單

`SERVICE_META` / `SERVICE_QUERY_FIELDS` 是 `Record<ServiceType, …>`，新增 union 成員時 TypeScript 會強迫補這兩張表；以下散點**不會**被型別系統帶出，需人工同步：

- `AutopassTickets.tsx` 的 `TABS`（普通陣列）——漏加的話新服務不會出現在任何 tab
- `TicketModals.tsx` 的 `isEtcToll`（etag-bound 失敗原因選項）與 `supportsCounter`（臨櫃子選項，目前僅強制險）
- 查繳平台外連判斷：`TicketCard.tsx` / `TicketModals.tsx` 以 `serviceMeta.platform === '街口支付'` 的 label 字串比較決定顯示純文字或 `platformUrl` 外連——平台名稱改字會靜默翻轉連結行為
- `autopassMock.ts` 各狀態示意票，與本文件 §3.3／§4.2 的服務對照表
...
- [ ] 回填 Modal「送出請款」進 Step 2 當下即跳「請款成功」toast，但 Step 2 仍可改選「失敗」（`TicketModals.tsx` 的 `handleAdvance`）——考慮把 toast 延後到結果確定時
...
```bash
npm install        # Node 22（同 CI，見 .nvmrc / engines）
npm run dev        # http://localhost:5173 或 5174
npm run typecheck  # tsc -b，純型別檢查（不打包，最快的驗證）
npm run lint
npm run build
```
